...
def getWordScore(word, n):
    ...
    if word == "":
        ...
        for letter in word:    
            out_score += SCRABBLE_LETTER_VALUES[letter]
        
        if L == n:
            bonus = 50
        
        out_score = out_score*L + bonus
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while(sum(hand.values()) > 0):
        ...
        # Ask user for input
        user_input = input("Enter word, or a \".\" to indicate that you are finished: ").lower()
